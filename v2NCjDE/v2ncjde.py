...
class DE:
    ...
    def euclidean_distance_full(self, alvo, k, dim):
        ...
        dist = []
        pop_aux = []
        pop_aux = sample(list(self.pop), len(self.pop)//2)
        

        for i in range(len(pop_aux)):
            s = 0
            if k == i:
    ...
    def euclidean_distance(self, alvo, k, dim, f, fpop):
        s = 0
        dist = []
        pop_aux = []
        pop_aux = list(self.pop)
        pop_aux = sample(list(self.pop), len(self.pop)//2)
        ...
        for i in range(len(pop_aux)):
            ...
            if k == i:
                ...
                #    dist_aux, aux = self.euclidean_distance(alvo, self.pop[i], dim, f, fpop)
                #    self.full_euclidean[i] = dist_aux
                #    fpop[i] = f.evaluate(self.pop[i])
                    #print(self.pop[i])
