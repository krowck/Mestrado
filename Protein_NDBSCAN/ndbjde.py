...
class DE:
    ...
    def diferentialEvolution(self, pop_size, dim, max_iterations, runs, func, f, nfunc, accuracy, flag_plot, eps_value, archive_flag, maximize=True):
        ...
        for r in range(runs):
            ...
            temp_best_archive = -9999
            for x in range(len(archive)):
                if fpop_archive[x] > temp_best_archive:
                    temp_best = fpop_archive[x]
                    indice_best_archive = x
                    temp_best_archive = fpop_archive[x]
                best_individual_archive = indice_best_archive
            


            #itermax = int((f.get_maxfes()*0.3/len(best_individuals))/dim)
            #itermax_archive = int((f.get_maxfes()*0.3/len(archive2))/dim)
            itermax_archive = 10000
            ...
            #print(fpop_archive)
            #print(archive2)
            #print(archive2[best_individual_archive])
            #print(np.asarray(archive2[best_individual_archive], dtype=np.float32))
            # endpt, it = nelder_mead(f, archive2[best_individual_archive])
            # print(endpt, it)
            # fpop = self.evaluatePopulation(archive2, func, f)
            # print(fpop)

            for ind in range(0,len(archive2)):
                it, endpt = nelder_mead(f, archive2[ind])
                archive2[ind] = it
            ...
            if archive_flag == 0:
                for ind in best_individuals:
                    it, endpt = hooke(dim, self.pop[ind], rho, eps, 10, f)                
                    self.pop[ind] = endpt
            #LOCAL SERACH ROUTINE WITH ARCHIVE
            elif archive_flag == 1: 
                print(fpop_archive[best_individual_archive])
                it, endpt = hooke(dim, archive2[best_individual_archive], rho, eps, itermax_archive, f) 
                #for ind in range(0,len(archive2)):
                    #find = 0
...
if __name__ == '__main__': 
    ...
    dim = f.get_dimension()
    max_iterations = int((f.get_maxfes() // pop_size) )
